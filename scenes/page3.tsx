			...
			<View style={{ alignItems: "center", justifyContent: "center", height: "100%", padding: "1%" }}>
				<View style={{ height: "100%", width: "100%", padding: "10%" }}>
					...
					<Button title={"DB2JSON"} onPress={() => {
						let tBalaList:{date:Date,kind:string,content:string,price:string,id:number}[] = [];
						balanceDataList.forEach((arow)=>{
							tBalaList.push(
								{
									date:arow.date,
									kind:arow.kind,
						...
						console.log(JSON.stringify(tBalaList, null, "\t"));
						Clipboard.setString(JSON.stringify(tBalaList, undefined, "\t"));
					}} />
					<Button title={"Json読込とDB更新"} onPress={() => {
						if (deleteTableCount === 3) {
							Clipboard.getString()
								.then((data) => {
									let newBalaData: {date:number,kind:string,content:string,price:string,id:number}[] = JSON.parse(data);

									newBalaData.forEach((baladata) => {
										Db.insertToDb(new BalanceData(
											new Date(baladata.date),
											baladata.kind,
											baladata.content,
											baladata.price,
											baladata.id
									...
									setDeleteTableCount(0);
									setSnackbarText("テーブルを更新しました。");
									setSnackbarButtonText("OK");
									setVisible(true);
								});
						} else {
							setDeleteTableCount(deleteTableCount + 1);
							setSnackbarText(`あと${3 - deleteTableCount}でテーブルを更新します。`);
							setSnackbarButtonText("CANCEL");
							setVisible(true);
						}
					}} />
